...
func (e *MapStateEntry) getNets(identities Identities, id identity.NumericIdentity) []*net.IPNet {
	if identities == nil {
		return nil
	}
	if id == identity.ReservedIdentityWorld {
		return []*net.IPNet{
			&net.IPNet{IP: net.IPv4zero, Mask: net.CIDRMask(0, net.IPv4len*8)},
			&net.IPNet{IP: net.IPv6zero, Mask: net.CIDRMask(0, net.IPv6len*8)},
		}
	}
...
func (keys MapState) denyPreferredInsertWithChanges(newKey Key, newEntry MapStateEntry, adds, deletes Keys, identities Identities) {
	...
	newEntryNets := newEntry.getNets(identities, identity.NumericIdentity(newKey.Identity))

	if newEntry.IsDeny {
		switch {
		// case for inserting a deny L3-only
		case newKey.DestPort == 0 && newKey.Nexthdr == 0 && newKey.Identity != 0:
			l4OnlyAllows := MapState{}
			// look for broad allows "all/unknown" identity and insert the same
			// entry, but with the specifc L3-deny identity. This ensures that
			// the broad allow is being overriden in the marginal circumstance of
			// as subset of its allow being denied by another policy.
			for k, v := range keys {
				vNets := v.getNets(identities, identity.NumericIdentity(k.Identity))
				if newKey.TrafficDirection == k.TrafficDirection &&
					!v.IsDeny &&
					(k.Identity == 0 || netsContainAny(newEntryNets, vNets)) &&
		...
		default:
			// We do not insert 'newKey' if the map state already denies traffic
		...
		for k, v := range keys {
			vNets := v.getNets(identities, identity.NumericIdentity(k.Identity))
			if k.TrafficDirection == newKey.TrafficDirection &&
				(newKey.DestPort == 0 || k.DestPort == newKey.DestPort) &&
				(newKey.Nexthdr == 0 || k.Nexthdr == newKey.Nexthdr) &&
				!v.IsDeny &&
				(k.Identity == newKey.Identity || netsContainAny(newEntryNets, vNets)) {
				keys.deleteKeyWithChanges(k, nil, adds, deletes)
			}
		}
		...
		// Do not insert L3-L4 if we have inserted a deny L3-only or mathcing L3-L4 deny.
		for k, v := range keys {
			vNets := v.getNets(identities, identity.NumericIdentity(k.Identity))
			if v.IsDeny && k.TrafficDirection == newKey.TrafficDirection &&
				(k.DestPort == 0 || k.DestPort == newKey.DestPort) &&
				(k.Nexthdr == 0 || k.Nexthdr == newKey.Nexthdr) &&
				(k.Identity == 0 || k.Identity == newKey.Identity || netsContainAny(vNets, newEntryNets)) {
				return
			}
		}
		if newKey.Identity == 0 && newKey.DestPort != 0 {
			// case for an existing deny L3-only and we are inserting allow L4-only
			for k, v := range keys {
				if newKey.TrafficDirection == k.TrafficDirection &&
					v.IsDeny && k.Identity != 0 && k.DestPort == 0 && k.Nexthdr == 0 {
...
func netsContainAny(a, b []*net.IPNet) bool {
	for _, an := range a {
		...
		for _, bn := range b {
			...
			if aIsIPv4 == bIsIPv4 {
				...
				if an.Contains(bn.IP) && bMask >= aMask {
					return true
				}
			}
		}
...
func (mc *MapChanges) consumeMapChanges(policyMapState MapState) (adds, deletes Keys) {
	...
	for i := range mc.changes {
		if mc.changes[i].Add {
			...
			policyMapState.denyPreferredInsertWithChanges(mc.changes[i].Key, mc.changes[i].Value, adds, deletes, nil)
		} else {
